...
class EventPublisherImpl(
    private val localEventDao: TimestampedEventDao,
    ...
    private suspend fun sendMessagesSequentially() {
        ...
        while (timestampedEvent != null) {
            try {
                val httpResultCode =
                    remote.send(Event(timestampedEvent.subject, timestampedEvent.payload))
                if (httpResultCode == 200) {
                    // log a successful send (in real project possibly avoid logging sensitive information here)
                    Log.i(
                        "EventPublisher",
                        "send was successful for ${timestampedEvent.subject} ${timestampedEvent.payload}"
                    )
                    ...
                    Log.e(
                        "EventPublisher",
                        "Server did not return 200 for event ${timestampedEvent.subject} ${timestampedEvent.payload}"
                    )
                    delay(1000)
